...
int CVICALLBACK InitializeTestMode1 (int panel, int event, void *callbackData,
		int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_GOT_FOCUS:
			...
			GetNumListItems (panel, TESTMODE1_TREE, &num_items);
			if(num_items==1) 
			{
				...
				for(i=0; i<=sys_config.MB; i++)
				{
					...
					idx_MB = InsertTreeItem (panel, TESTMODE1_TREE, VAL_CHILD, 0, VAL_LAST, item_label_MB, item_label, 0, num_items++);
					...
					for(j=0; j<=sys_config.DUC; j++)	  // assumes same number of DUC into each MB
					{
						...
						idx_DUC = InsertTreeItem (panel, TESTMODE1_TREE, VAL_CHILD, idx_MB, VAL_LAST, item_label_DUC, item_label, 0, num_items++);
						...
						for(k=0; k<=sys_config.DB; k++)		 // assumes same number of DB into each DUC
						{
							...
							idx_DB = InsertTreeItem (panel, TESTMODE1_TREE, VAL_CHILD, idx_DUC, VAL_LAST, item_label_DB, item_label, 0, num_items++);
							strcpy(item_label,item_label_root);   // tag = MB0DUC0
							if(k == current_location.DB)
								SetTreeItemAttribute (panel, TESTMODE1_TREE, idx_DB, ATTR_SELECTED, 1);
						}
						strcpy(item_label,item_label_MB);   // tag = MB0
						if(j != current_location.DUC)
							SetTreeItemAttribute (panel, TESTMODE1_TREE, idx_DUC, ATTR_COLLAPSED, 1);
			
					}
					strcpy(item_label,"");   // tag = ""
					if(i != current_location.MB)
						SetTreeItemAttribute (panel, TESTMODE1_TREE, idx_MB, ATTR_COLLAPSED, 1);
				}
			} 
			else 
			{
				...
				{
					// collapse all
					SetTreeItemAttribute (panel, TESTMODE1_TREE, i, ATTR_COLLAPSED, 1);	
				}
				...
				if(current_location.MB != -1)
				{
					...
					GetTreeItemFromTag (panel, TESTMODE1_TREE, item_label, &idx_MB);
					SetTreeItemAttribute (panel, TESTMODE1_TREE, idx_MB, ATTR_COLLAPSED, 0);	
				} 
				if(current_location.DUC != -1)
				{
					sprintf(item_label, "MB%dDUC%d", current_location.MB, current_location.DUC);
					GetTreeItemFromTag (panel, TESTMODE1_TREE, item_label, &idx_DUC);
					SetTreeItemAttribute (panel, TESTMODE1_TREE, idx_DUC, ATTR_COLLAPSED, 0);	
				} 
				if(current_location.DB != -1)
				{
					sprintf(item_label, "MB%dDUC%dDB%d", current_location.MB, current_location.DUC, current_location.DB);
					GetTreeItemFromTag (panel, TESTMODE1_TREE, item_label, &idx_DB);
					SetTreeItemAttribute (panel, TESTMODE1_TREE, idx_DB, ATTR_SELECTED, 1);
				} 
			}
			
			//SetActiveCtrl(panel, TESTMODE1_TREE);
			break;
...
int CVICALLBACK TestMode1Tree (int panel, int control, int event,
