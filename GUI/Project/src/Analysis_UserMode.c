...
int CVICALLBACK InitializeUserMode (int panel, int event, void *callbackData,
		int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_GOT_FOCUS:
			sprintf(title_string, "User Mode - MB%d DUC%d DB%d", current_location.MB, current_location.DUC, current_location.DB);
			SetPanelAttribute (panel, ATTR_TITLE, title_string); 
		
			// populate instrument tree
			GetNumListItems (panel, USERMODE_TREE, &num_items);
			if(num_items==1) 
			{
				...
				for(i=0; i<=sys_config.MB; i++)
				{
					...
					idx_MB = InsertTreeItem (panel, USERMODE_TREE, VAL_CHILD, 0, VAL_LAST, item_label_MB, item_label, 0, num_items++);
					...
					for(j=0; j<=sys_config.DUC; j++)	  // assumes same number of DUC into each MB
					{
						...
						idx_DUC = InsertTreeItem (panel, USERMODE_TREE, VAL_CHILD, idx_MB, VAL_LAST, item_label_DUC, item_label, 0, num_items++);
						...
						for(k=0; k<=sys_config.DB; k++)		 // assumes same number of DB into each DUC
						{
							...
							idx_DB = InsertTreeItem (panel, USERMODE_TREE, VAL_CHILD, idx_DUC, VAL_LAST, item_label_DB, item_label, 0, num_items++);
							strcpy(item_label,item_label_root);   // tag = MB0DUC0
							if(k == current_location.DB)
								SetTreeItemAttribute (panel, USERMODE_TREE, idx_DB, ATTR_SELECTED, 1);
						}
						strcpy(item_label,item_label_MB);   // tag = MB0
						if(j != current_location.DUC)
							SetTreeItemAttribute (panel, USERMODE_TREE, idx_DUC, ATTR_COLLAPSED, 1);
			
					}
					strcpy(item_label,"");   // tag = ""
					if(i != current_location.MB)
						SetTreeItemAttribute (panel, USERMODE_TREE, idx_MB, ATTR_COLLAPSED, 1);
				}
			} 
			else 
			{
				...
				{
					// collapse all
					SetTreeItemAttribute (panel, USERMODE_TREE, i, ATTR_COLLAPSED, 1);	
				}
				...
				if(current_location.MB != -1)
				{
					...
					GetTreeItemFromTag (panel, USERMODE_TREE, item_label, &idx_MB);
					SetTreeItemAttribute (panel, USERMODE_TREE, idx_MB, ATTR_COLLAPSED, 0);	
				} 
				if(current_location.DUC != -1)
				{
					sprintf(item_label, "MB%dDUC%d", current_location.MB, current_location.DUC);
					GetTreeItemFromTag (panel, USERMODE_TREE, item_label, &idx_DUC);
					SetTreeItemAttribute (panel, USERMODE_TREE, idx_DUC, ATTR_COLLAPSED, 0);	
				} 
				if(current_location.DB != -1)
				{
					sprintf(item_label, "MB%dDUC%dDB%d", current_location.MB, current_location.DUC, current_location.DB);
					GetTreeItemFromTag (panel, USERMODE_TREE, item_label, &idx_DB);
					SetTreeItemAttribute (panel, USERMODE_TREE, idx_DB, ATTR_SELECTED, 1);
				} 
			}
			
			//SetActiveCtrl(panel, USERMODE_TREE);
			break;
